	...
	// 			var lang = ('lang' in curPrefs) ? curPrefs.lang : null;
			...
			$.svgIcons(curConfig.imgPath + 'svg_edit_icons.svg', {
				...
				placement: {
					...
					'#tool_download div div':'import',
					'#tool_import div div':'import',
					'#tool_source':'source',
					'#tool_upload div div':'save',
...
// 					console.log('cm', $(evt.target).closest('.contextMenu'));
			...
			*/
			...
			// MKREITLER: support for downloading SVG data from the database.
			var clickDownload = function(){
				var resourceName = prompt("Please enter the SVG to load","mySVG");
				if (resourceName) {

					svgIO.loadFromDatabase(resourceName, function(data) {
						svgCanvas.clear();
						loadSvgString(data);
						updateCanvas();
						}, function(filename) { alert("Failed to load " + filename); }, this);
				}
			};
...
// 					}
			...
			(function() {
				var toolnames = ['clear','open','download','save','source','delete','delete_multi','paste','clone','clone_multi','move_top','move_bottom'];
				var all_tools = '';
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
		...
		// 	}
			...
			var Actions = function() {
				...
				var tool_buttons = [
					...
					{sel:'#tool_download', fn: clickDownload, evt: 'mouseup', key: ['W', true]},
					{sel:'#tool_import', fn: clickImport, evt: 'mouseup'},
					{sel:'#tool_upload', fn: clickUpload, evt: 'mouseup', key: ['U', true]},
					{sel:'#tool_source', fn: showSourceEditor, evt: 'click', key: ['C', true]},
		...
		// 			{sel:'#tools_ellipse_show', fn: clickEllipse, evt: 'click'},
			...
			if (window.FileReader) {
				...
				var open = $('<input type="file">').on('change', function() {
					var f = this;
